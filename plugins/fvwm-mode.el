...
;; 
;; Thanks also to Hun from #fvwm for testing, bugreporting and providing patches with added keywords.
;; 
;; ChangeLog:
;; 1.6.2
...
;; 
(defconst fvwm-mode-version "1.6.2"
...
(defvar fvwm-fvwmcommand-path "/usr/bin/FvwmCommand"
...
(defvar fvwm-syntax-table
  ...
  (let ((table (copy-syntax-table)))
     ...
     (modify-syntax-entry ?\- "w"   table) ;otherwise it'll highlight parts of strings containing an Fvwm keyword.
...
(defvar fvwm-functions '(
	...
	"Echo" "EchoFuncDefinition" "EdgeCommand" "EdgeMoveResistance" "EdgeMoveDelay" "EdgeResizeDelay" "EdgeResistance" "EdgeScroll" "EdgeThickness" "Emulate"
...
(defvar fvwm-keywords-1 '(
       ...
       "InActive" "IndexedWindowName" "InitialMapCommand" "Interior" "Item" "ItemFormat" "Iterations"
...
(defvar fvwm-font-lock-keywords fvwm-font-lock-keywords-2 "Default highlighting for Fvwm mode")
        ...
        (let ((fvwm-keywords-all (append fvwm-functions fvwm-keywords-1 fvwm-keywords-2)))
          ...
          (if (and (not (featurep 'xemacs)) (> emacs-major-version 21))
              ...
              (progn
                ...
                (let ((i 0) (cur))
                  (while (< i (length fvwm-keywords-all))
                    (mapc (lambda (e) (add-to-list 'fvwm-keywords-map (cons e nil))) fvwm-keywords-all) ;thank you fledermaus!
                    (setq i (+ i 1))))))))
...
Entry to this mode calls the value of `fvwm-mode-hook'"
  ...
  (fvwm-disable-indentation)            ;not working decently
